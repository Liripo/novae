...
`--font`, then the `NOVAE_CJK_FONT` env var, then common system paths
(Windows `C:/Windows/fonts/msyh.ttc` / `simhei.ttf`, macOS
`/System/Library/Fonts/PingFang.ttc`, Linux `fc-list :lang=zh`).
Do not download fonts into the task directory. If no font is found, pass an
explicit compatible `.ttf`.
