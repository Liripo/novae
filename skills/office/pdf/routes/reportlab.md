...
ReportLab requires explicit font registration for CJK text. Locate a system CJK TrueType font instead of downloading fonts into the workspace:
...
def _find_cjk_font() -> str:
    """按平台查找常见 CJK 字体，找不到则抛出明确错误。"""
    candidates = [
        os.environ.get('NOVAE_CJK_FONT', ''),
    ...
    for path in candidates:
        if path and os.path.isfile(path):
            return path
    raise RuntimeError('No CJK font found; pass an explicit .ttf path')

regular_font = _find_cjk_font()
...
Use TrueType `glyf` fonts (`.ttf` / `.ttc`) that ReportLab `TTFont` supports; do not pass CFF/PostScript-outline OTF assets to `TTFont`. Do not copy or download fonts into the task directory.
